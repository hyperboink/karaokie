...
function PinnedRow({ item, isDragOverlay = false }: { item: QueueItem; isDragOverlay?: boolean }) {
  ...
  const rowClass = [
    'group flex items-center gap-3 px-3 py-2.5 landscape:py-1.5 rounded-xl border transition-all select-none outline-none',
    isDragging
      ? 'opacity-0'
      : isDragOverlay
      ? 'bg-gray-700 border-yellow-400/40 shadow-xl shadow-black/40 cursor-grabbing'
      : 'bg-gray-800/40 hover:bg-gray-800/70 border-gray-700/30 hover:border-gray-700/60 cursor-grab active:cursor-grabbing',
  ].join(' ');

  return (
    <div ref={setNodeRef} style={rowStyle} {...attributes} {...listeners} className={rowClass}>
      {/* Drag handle indicator */}
      <svg className="w-3 h-3 text-gray-600 shrink-0" fill="currentColor" viewBox="0 0 24 24">
      ...
      >
        <button
          ...
          className="p-1.5 text-yellow-400 bg-yellow-400/10 hover:bg-yellow-400/20 active:bg-yellow-400/30 transition-colors rounded-lg opacity-0 group-hover:opacity-100 md:opacity-0 md:group-hover:opacity-100"
          title="Add to queue"
        >
        ...
        <button
          ...
          className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-red-400/10 active:bg-red-400/10 transition-colors rounded-lg opacity-0 group-hover:opacity-100 md:opacity-0 md:group-hover:opacity-100"
          title="Unpin"
        >
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      >
        ...
        <button
          onClick={() => togglePin(item)}
          className="p-1.5 text-gray-500 active:text-red-400 transition-colors rounded-lg"
          title="Unpin"
        >
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
    ...
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <div className="flex items-center justify-between px-4 landscape:px-3 py-2.5 landscape:py-1.5 border-b border-gray-800">
        <div className="flex items-center gap-2">
          ...
          {pinned.length > 0 && (
            <span className="text-[9px] text-gray-500 font-medium">{pinned.length} songs</span>
          )}
        </div>
        <button
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-3 md:px-4 landscape:px-2 py-3 landscape:py-2">
        {pinned.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 text-center gap-2">
            ...
            <p className="text-gray-500 text-sm font-medium">No pinned songs</p>
            <p className="text-gray-600 text-xs">Pin your favourites from the queue.</p>
          </div>
        ) : (
          <DndContext
            sensors={sensors}
